...
# ALTERNATIVE COSINE SIMILARITY - VECTORSL 
def XosineSimilarity(X_set, Y_set):
    l1 = []
    l2 = []
    rvector = X_set + Y_set
    for w in rvector: 
...
print(files['computer.txt'][2].most_common(30))
for X, Y in combinations(filesKeys, 2):
    print("Cosine similarity of " + str(X) + " and " + str( Y ) + " ", XosineSimilarity(files[X][4], files[Y][4]))
